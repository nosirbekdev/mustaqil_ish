...
@include('components.header')
    ...
    <x-article-search-form
        ...
        subtitle="Monografiya qidirish uchun ma`lumotlarni to`ldiring"
        action="{{ route('monographs.index') }}"
        option="Nashriyotni tanlang"
        ...
        <div class="max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
            @foreach ($monographs as $book)
                <div class="flex items-center gap-4 bg-sky-50/50 p-4 rounded-lg hover:bg-sky-50/80 transition-colors">
                    ...
                    <button class="bg-indigo-900 hover:bg-indigo-800 text-white shrink-0 py-2 px-6 rounded-md"><a href="{{ asset('storage/' . $book->pdf) }}">Monograf</a></button>
                </div>
            @endforeach
        </div>
    @elseif(isset($monographs) && $books->isNotEmpty())
