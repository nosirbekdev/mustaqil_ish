...
@include('components.header')
    ...
    subtitle="Dissertatsiya qidirish uchun ma`lumotlarni to`ldiring"
    action="{{ url('user/about/dissertations') }}"
...
/>
    ...
    @if (!request()->has('q') && !request()->has('search_publisher') && !request()->has('search_language'))
        <h2 class="text-2xl font-semibold text-gray-700 mb-4">Barcha dissertatsiyalar</h2>
        <div class="max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
            @foreach ($dissertations as $book)
                <div class="flex items-center gap-4 bg-sky-50/50 p-4 rounded-lg hover:bg-sky-50/80 transition-colors">
                    ...
                    <button class="bg-indigo-900 hover:bg-indigo-800 text-white shrink-0 py-2 px-6 rounded-md"><a href="{{ asset('storage/' . $book->pdf) }}">Dissertatsiyalar</a></button>
                </div>
            @endforeach
        </div>
