...
@section('title', 'Jurnalni tahrirlash')
        ...
        <div class="w-full max-w-2xl">
            <div class="bg-white shadow-md rounded-lg">
                <div class="bg-blue-600 text-white p-4 rounded-t-lg">
                    <h4 class="text-lg font-semibold">Jurnalni tahrirlash</h4>
                </div>
                <div class="p-6">
                <form action="{{ route('journal.update', $journal->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-5">
                        <label for="content" class="block text-sm font-medium text-gray-700">Jurnal matni</label>
                        <textarea
                            name="content"
                            id="content"
                            rows="10"
                    ...
                    <div class="mb-5">
                        ...
                        >
                        @if ($journal->image)
                            <p class="text-sm text-gray-500 mt-2">Joriy rasm: <a href="{{ asset('storage/' . $journal->image) }}" target="_blank" class="text-blue-600 hover:underline">Ko'rish</a></p>
                        @endif
                        @error('image')
                            <p class="text-sm text-red-500 mt-2">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-5">
                        ...
                        >
                        @if ($journal->pdf)
                            <p class="text-sm text-gray-500 mt-2">Joriy PDF: <a href="{{ asset('storage/' . $journal->pdf) }}" target="_blank" class="text-blue-600 hover:underline">Ko'rish</a></p>
                        @endif
                        @error('pdf')
                            <p class="text-sm text-red-500 mt-2">{{ $message }}</p>
                        @enderror
