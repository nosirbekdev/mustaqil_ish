    ...
    <div class="flex justify-center">
        <div class="w-full max-w-2xl">
            <div class="bg-white shadow-md rounded-lg">
                ...
                <div class="p-6">
                <form action="{{ route('europes.update', $europe->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-5">
                        ...
                        <textarea
                            ...
                            required
                        >{{ old('content', $europe->content) }}</textarea>
                        @error('content')
                            <p class="text-sm text-red-500 mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        >
                        @if ($europe->image)
                            <p class="text-sm text-gray-500 mt-2">Joriy rasm: <a href="{{ asset('storage/' . $europe->image) }}" target="_blank" class="text-blue-600 hover:underline">Ko'rish</a></p>
                        @endif
                        @error('image')
                            <p class="text-sm text-red-500 mt-2">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-5">
                        ...
                        >
                        @if ($europe->pdf)
                            <p class="text-sm text-gray-500 mt-2">Joriy PDF: <a href="{{ asset('storage/' . $europe->pdf) }}" target="_blank" class="text-blue-600 hover:underline">Ko'rish</a></p>
                        @endif
                        @error('pdf')
                            <p class="text-sm text-red-500 mt-2">{{ $message }}</p>
                        @enderror
